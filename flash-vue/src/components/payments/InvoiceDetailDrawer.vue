...
async function handleDownload() {
  ...
  try {
    ...
    downloadData.value = {
      ...
      company: { name: 'Flash AI', email: 'billing@flash.ai' },
    }
  } finally {
...
function buildMockInvoice(id: number): InvoiceDetail {
  return {
    ...
    footer: 'Generated by Flash AI Billing.',
      ...
      <div class="hero-card">
        <div class="hero-row">
          <div class="hero-main">
            ...
            <p class="hero-sub">{{ detail.user.name }} · {{ detail.payment?.uuid || 'No payment linked' }}</p>
        ...
        <div class="stats-grid">
          <article class="stat-card">
            <span class="stat-k">Subtotal</span>
          ...
          <article class="stat-card">
            <span class="stat-k">Tax</span>
            <strong>{{ formatMoney(detail.tax_amount, detail.currency) }}</strong>
            <small>{{ formatMoney(detail.total, detail.currency) }} total</small>
          </article>
          <article class="stat-card">
            <span class="stat-k">Due</span>
            <strong>{{ formatDateTime(detail.due_at) }}</strong>
            <small>Paid {{ formatDateTime(detail.paid_at) }}</small>
          </article>
          <article class="stat-card">
            <span class="stat-k">Items</span>
            <strong>{{ detail.line_items?.length || 0 }}</strong>
            <small>{{ detail.subscription?.plan.name || 'No plan' }}</small>
          </article>
        </div>
      </div>
        ...
        <TabPanels>
          <TabPanel value="overview">
            <div class="section-grid">
              ...
              <section class="info-card">
                ...
                <div class="meta-list">
                  <div class="meta-row"><span>Payment</span><span>{{ detail.payment?.uuid || '—' }}</span></div>
                  <div class="meta-row"><span>Gateway</span><span>{{ detail.payment?.payment_gateway || '—' }}</span></div>
              ...
              <div class="mini-list stack-list">
                <div v-for="(item, index) in detail.line_items || []" :key="`${item.description}-${index}`" class="stack-row">
                  <div>
                    ...
                    <span class="row-sub">Qty {{ item.quantity }} · Unit {{ formatMoney(item.unit_price, detail.currency) }}</span>
                  </div>
                  <div class="row-meta">
          ...
          <TabPanel value="download">
            <div class="section-head-inline">
              <h3 class="section-title">Download Payload</h3>
              <Button label="Fetch Download Data" size="small" :loading="downloading" @click="handleDownload" />
            </div>
            ...
            <div class="payload-grid">
              ...
              <section class="payload-card">
                <h3 class="section-title">Invoice Payload</h3>
